...
class Guessr:
    ...
    def get_guess(self, difficulty):
        ...
        random_chamber = random.choice(Guessr.chambers_list)
        ...
        random_index = random.randint(0, len(image_files))
        random_path = os.path.join(image_path, image_files[random_index])  # Gets the path for the random image.
...
class GuessrUsersStatistics:
    ...
    def add_user_stats(self, server_id, user_id, difficulty):
        ...
        if user_id not in self.stats[server_id]:
            self.stats[server_id][user_id] = {"Easy": 0, "Medium": 0, "Hard": 0}
    ...
    def delete_user_stats(self, server_id, user_id):
        ...
        if server_id not in self.stats:
            raise KeyError("Server not found in statistics.")
        key_to_remove = None
        for key in self.stats[server_id]:
            if user_id == key:
                key_to_remove = key
        if key_to_remove:
